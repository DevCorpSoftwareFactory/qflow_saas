...
RETURNS TRIGGER AS $$
BEGIN
    IF (SELECT COUNT(*) FROM chart_of_accounts WHERE tenant_id = NEW.tenant_id) = 0 THEN
        -- Insertar PUC básico
        INSERT INTO chart_of_accounts (tenant_id, code, name, account_type, level, nature, parent_id) VALUES
            -- 1xx - ACTIVOS
            (NEW.tenant_id, '11', 'ACTIVOS', 'asset', 1, 'debit', NULL),
            (NEW.tenant_id, '1105', 'CAJA', 'asset', 2, 'debit', NULL),
            (NEW.tenant_id, '110505', 'Caja General', 'asset', 3, 'debit', NULL),
            (NEW.tenant_id, '1110', 'BANCOS', 'asset', 2, 'debit', NULL),
            (NEW.tenant_id, '111005', 'Cuenta Corriente', 'asset', 3, 'debit', NULL),
            (NEW.tenant_id, '1305', 'CLIENTES', 'asset', 2, 'debit', NULL),
            (NEW.tenant_id, '130505', 'Cuentas por Cobrar', 'asset', 3, 'debit', NULL),
            (NEW.tenant_id, '1435', 'MERCANCÍAS', 'asset', 2, 'debit', NULL),
            (NEW.tenant_id, '143505', 'Inventario Productos', 'asset', 3, 'debit', NULL),
            -- 2xx - PASIVOS
            (NEW.tenant_id, '21', 'PASIVOS', 'liability', 1, 'credit', NULL),
            (NEW.tenant_id, '2105', 'PROVEEDORES', 'liability', 2, 'credit', NULL),
            (NEW.tenant_id, '210505', 'Cuentas por Pagar', 'liability', 3, 'credit', NULL),
            (NEW.tenant_id, '2408', 'IVA POR PAGAR', 'liability', 2, 'credit', NULL),
            -- 4xx - INGRESOS
            (NEW.tenant_id, '41', 'INGRESOS', 'income', 1, 'credit', NULL),
            (NEW.tenant_id, '4135', 'VENTAS', 'income', 2, 'credit', NULL),
            (NEW.tenant_id, '413505', 'Ventas de Mercancía', 'income', 3, 'credit', NULL),
            -- 5xx - GASTOS
            (NEW.tenant_id, '51', 'GASTOS', 'expense', 1, 'debit', NULL),
            (NEW.tenant_id, '5105', 'GASTOS DE PERSONAL', 'expense', 2, 'debit', NULL),
            (NEW.tenant_id, '5135', 'GASTOS GENERALES', 'expense', 2, 'debit', NULL);
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
