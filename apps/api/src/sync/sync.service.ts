...
import { CashSession, CashMovement } from '@qflow/database';
...
export class SyncService {
    ...
    if (dto.cashSessions && dto.cashSessions.length > 0) {
      for (const sessionDto of dto.cashSessions) {
        try {
          ...
          this.logger.error(
            `Failed to sync cash session ${sessionDto.id}`,
            error,
          );
          result.errors!.push(
    ...
    if (dto.cashMovements && dto.cashMovements.length > 0) {
      for (const movementDto of dto.cashMovements) {
        try {
          ...
          this.logger.error(
            `Failed to sync cash movement ${movementDto.id}`,
            error,
          );
          result.errors!.push(
  ...
  private async processOfflineCashMovement(
    ...
    await this.dataSource.transaction(async (manager: EntityManager) => {
      ...
      const delta =
        dto.movementType === 'income' || dto.movementType === 'sale'
          ? dto.amount
          : -dto.amount;
